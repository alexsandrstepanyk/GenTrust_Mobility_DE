...
#   - Формат запису: Дата, Версія, Опис змін
...
# Перевірка порту в діапазоні 3000-9000
check_port_in_range() {
    local port=$1
    if [ $port -lt 3000 ] || [ $port -gt 9000 ]; then
        echo -e "${RED}❌ ПОРТ $port поза діапазоном 3000-9000!${NC}"
        echo -e "${RED}   ЗАБОРОНЕНО чіпати порти поза цим діапазоном!${NC}"
        return 1
    fi
    return 0
}

# Функція запуску з перевіркою порту
launch_service() {
    ...
    while [ $retry -lt $max_retries ]; do
        ...
        if [ -n "$existing_pid" ]; then
            ...
            if lsof -ti:$port 2>/dev/null | grep -q .; then
                ...
                echo -e "${RED}   Рішення: Вбийте вручну: sudo lsof -ti:$port | xargs sudo kill -9${NC}"
                return 1
            else
...
# 1. Backend API (ЗАВЖДИ ПЕРШИМ) - порт 3000
echo -e "${CYAN}[1/2] Backend API v6 (порт 3000)${NC}"
...
echo -e "   ✅ Fixed ports (3000 = Backend API)"
echo -e "   ✅ Code preservation (no deletions)"
